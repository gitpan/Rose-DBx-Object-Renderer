...
sub config
{
	...
	{		
		$self->{CONFIG} = {
			...
			misc => {time_zone => 'Australia/Sydney', stringify_delimiter => ' ', doctype => '<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">', html_head => '<style type="text/css">body,div,dl,dt,dd,ul,ol,li,h1,h2,h3,h4,h5,h6,pre,form,fieldset,input,textarea,p,blockquote,th,td{margin:0;padding:0;}table{border-collapse:collapse;border-spacing:0;}fieldset,img{border:0;}address,caption,cite,code,dfn,em,strong,th,var{font-style:normal;font-weight:normal;}ol,ul{list-style:none;}caption,th{text-align:left;}h1,h2,h3,h4,h5,h6{font-size:100%;font-weight:normal;}q:before,q:after{content:\'\';}abbr,acronym{border:0;}body{font-size:93%;font-family:"Lucida Grande",Helvetica,Arial,Verdana,sans-serif;color:#222;}a,a:hover{color:#1B80BB;text-decoration:none;}a:hover{color:#0D3247;}p{padding:10px 20px;}form td{border:0px;text-align:left;}form tr:hover{background-color:#fff;}form table td span,label span{color:red;}label{color:#333;}input,textarea,select{font-size:100%;font-family:"Lucida Grande",Helvetica,Arial,Verdana,sans-serif;color:#333;background-color:#fcfcfc;border:1px solid #ddd;margin:0px 5px;padding:4px 8px;-moz-border-radius:4px;-webkit-border-radius:4px;}input[type="radio"],input[type="checkbox"]{border:0px;background:transparent;}input[type="submit"]{font-size:108%;padding:4px 8px;-moz-border-radius:5px;-webkit-border-radius:5px;cursor:pointer;background-color:#eee;background:-moz-linear-gradient(top,#fff 0%,#eee 30%,#ddd 100%);background:-webkit-gradient(linear, left top, left bottom, from(#fff), to(#ddd), color-stop(0.3, #eee));-webkit-transition:-webkit-box-shadow 0.1s linear;text-shadow:0px 1px 1px #fff;}input:hover[type="submit"]{background:#d0d0d0;color:#0D3247;background:-moz-linear-gradient(top,#f0f0f0,#c0c0c0);background:-webkit-gradient(linear,left top,left bottom,from(#f0f0f0), to(#c0c0c0));}input:active[type="submit"]{-webkit-box-shadow:0 0 5px #333;-moz-box-shadow:0 0 5px #333;}h1,h2{font-size:350%;padding:15px;text-shadow: 0px 1px 2px #aaa;}p{padding:10px 20px;}div{padding:10px 10px 0px 10px;}table{padding:5px 10px;width:100%;}th,td{padding:14px 6px;border-bottom: 1px dotted #ddd;font-size:85%;}th{color:#666;font-size:108%;font-weight:normal;border:0;background-color:#e0e0e0;background:-moz-linear-gradient(top,#f0f0f0 0%,#d0d0d0 80%,#cfcfcf 100%);background:-webkit-gradient(linear,left top,left bottom,from(#f3f3f3),to(#cfcfcf),color-stop(0.8, #d0d0d0));text-shadow: 0px 1px 1px #fff;}tr{background-color:#fff;-webkit-transition:background-color 0.3s linear}tr:hover{background-color:#F5F9FA;}.block{padding:5px;text-align:right;font-size:108%;}.menu{background-color:#e3e3e3;background:-moz-linear-gradient(top,#f0f0f0 0%,#e0e0e0 60%,#ddd 100%);background:-webkit-gradient(linear,left top,left bottom,from(#f0f0f0),to(#ddd),color-stop(0.6,#e0e0e0));padding:0px;width:100%;height:37px;-moz-border-radius-topleft:5px;-moz-border-radius-topright:5px;-webkit-border-top-left-radius:5px;-webkit-border-top-right-radius:5px;}.menu ul{padding:10px 6px 0px 6px;}.menu ul li{display:inline;}.menu ul li a{text-shadow: 0px 1px 1px #fff;float:left;display:block;color:#555;background-color:#d0d0d0;text-decoration:none;margin:0px 4px;padding:6px 18px;height:15px;-moz-border-radius-topleft:5px;-moz-border-radius-topright:5px;-webkit-border-top-left-radius:5px;-webkit-border-top-right-radius:5px;-webkit-transition:background-color 0.2s linear;}.menu ul li a:hover{background-color:#f0f0f0;color:#0D3247;}.menu ul li a:active{background-color:#fff;color:#1B80BB;}.menu ul li a.current,.menu ul li a.current:hover{cursor:pointer;background-color:#fff;}.pager{display:block;float:left;padding:2px 6px;border:1px solid #d0d0d0;margin-right:1px;-moz-border-radius:2px;-webkit-border-radius:2px;-webkit-transition: border 0.5s linear;}a.pager:hover{border:1px solid #0D3247;}</style>'},
			columns => {
		...
		$self->{CONFIG}->{columns}->{'doubleprecision'} = $self->{CONFIG}->{columns}->{'numeric'};
		$self->{CONFIG}->{columns}->{'decimal'} = $self->{CONFIG}->{columns}->{'numeric'};
		$self->{CONFIG}->{columns}->{'bigint'} = $self->{CONFIG}->{columns}->{'integer'};
		$self->{CONFIG}->{columns}->{'serial'} = $self->{CONFIG}->{columns}->{'integer'};
		$self->{CONFIG}->{columns}->{'bigserial'} = $self->{CONFIG}->{columns}->{'integer'};
		$self->{CONFIG}->{columns}->{'quantity'} = $self->{CONFIG}->{columns}->{'integer'};
		$self->{CONFIG}->{columns}->{'height'} = $self->{CONFIG}->{columns}->{'length'};
		$self->{CONFIG}->{columns}->{'width'} = $self->{CONFIG}->{columns}->{'length'};
		$self->{CONFIG}->{columns}->{'depth'} = $self->{CONFIG}->{columns}->{'length'};
		$self->{CONFIG}->{columns}->{'title'} = $self->{CONFIG}->{columns}->{'name'};
		$self->{CONFIG}->{columns}->{'birth'} = $self->{CONFIG}->{columns}->{'date'};
		$self->{CONFIG}->{columns}->{'fax'} = $self->{CONFIG}->{columns}->{'phone'};
		$self->{CONFIG}->{columns}->{'cost'} = $self->{CONFIG}->{columns}->{'money'};
		$self->{CONFIG}->{columns}->{'price'} = $self->{CONFIG}->{columns}->{'money'};
		$self->{CONFIG}->{columns}->{'blob'} = $self->{CONFIG}->{columns}->{'text'};
		$self->{CONFIG}->{columns}->{'comment'} = $self->{CONFIG}->{columns}->{'text'};
		$self->{CONFIG}->{columns}->{'file'} = $self->{CONFIG}->{columns}->{'document'};
				...
				unless ($column->{is_primary_key_member})
				{
					...
					{	
						...
						if (exists $foreign_keys->{$column})
						{
							...
							$config->{columns}->{$column}->{format}->{for_view} = sub {my ($self, $column) = @_;return unless $self->$column;return $self->$foreign_object_name->stringify_me;} unless exists $config->{columns}->{$column}->{format} && exists $config->{columns}->{$column}->{format}->{for_view};
						}
					}
